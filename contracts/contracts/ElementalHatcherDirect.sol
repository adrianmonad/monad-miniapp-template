...
/**
 * @title ElementalHatcherDirect
 * @dev Contract for hatching Elemental Adventurers by claiming NFTs from a treasury
 * Users pay directly to the treasury wallet, not to this contract
 */
contract ElementalHatcherDirect is Ownable, ReentrancyGuard {
    // Constants
    uint256 public constant MAX_SUPPLY = 10000; // Total NFTs (0-9999)
    uint256 public constant HATCH_PRICE = 0.1 ether; // Reference price, not collected by contract
    ...
    /**
     * @dev Claim an adventurer after paying the treasury directly
     * Randomly selects an unclaimed NFT and transfers it from treasury to caller
     * Note: This contract doesn't handle payments, users must pay the treasury directly
     */
    function claimAdventurer() external nonReentrant {
