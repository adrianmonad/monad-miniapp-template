...
async function main() {
  ...
  console.log("Deploying ElementalHatcher contract...");
  console.log(`NFT Contract: ${NFT_CONTRACT_ADDRESS}`);
  console.log(`Treasury: ${TREASURY_ADDRESS}`);

  // Deploy the ElementalHatcher contract
  const ElementalHatcher = await hre.ethers.getContractFactory("ElementalHatcher");
  const hatcher = await ElementalHatcher.deploy(NFT_CONTRACT_ADDRESS, TREASURY_ADDRESS);

  await hatcher.waitForDeployment();

  const hatcherAddress = await hatcher.getAddress();
  console.log(`ElementalHatcher deployed to: ${hatcherAddress}`);
  console.log("");
